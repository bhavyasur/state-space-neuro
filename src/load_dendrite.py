"""
Description:  Loads a single session of data for a given mouse into Jax data structures 
              for processing with Dynamax
...
def concatenate_sessions(cond_set):
    for i in set:
        ...
        behavior_array = jnp.concatenate(behavior)
        calcium_array = jnp.concatenate(calcium)
        manual_array = jnp.concatenate(manual)
