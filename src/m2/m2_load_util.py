...
def plot_zhatlem_cue(trial_break, zhat_lem, Fs, retained_trial_idx, disc_states, 
    ...
    if m2_sensor:
        ...
        for i in range(len(retained_trials)):
            ...
            trial_cue = trial["CuePlayed"] // bin_size
            before = int(trial_cue - ((3 * Fs) // bin_size))
            after = int(trial_cue + ((3 * Fs) // bin_size))
    ...
    if m2_sensor:
        ...
        ax.axvline(((3*Fs) // bin_size), lw=1.25, color='r', linestyle='--', label='Cue')
...
def plot_zhatlem_probability_cue(trial_break, zhat_lem, Fs, retained_trial_idx, disc_states, bin_size,
...
def plot_zhatlem_probability_lick(trial_break, zhat_lem, Fs, retained_trial_idx, disc_states, bin_size,
                             m2_sensor: Literal['WM1', 'Gate', 'WM2', 'CuePlayed', 'Lick', None] = None):
